...
public:
     ...
     */
    // static const int NNPSMEFTd6Vars = 547;
    static const int NNPSMEFTd6Vars = 553;
     ...
     */
    // static const int NNPSMEFTd6Vars_LFU_QFU = 287;
    static const int NNPSMEFTd6Vars_LFU_QFU = 293;
    ...
    */
    ...
    /**
     * @brief A function calculating the theoretical uncertainty in the ratio @f$\mu_{eeZH}@f$,
     * dependent on the value of @f$\kappa_\lambda@f$, at the FCC-ee with @f$\sqrt{s}=240@f$ GeV.
     * @param[in] klam is the value of @f$\kappa_\lambda@f$, a, b and c are the coefficients of the 
     * quadratic function of klam
     * @return @f$\epsilon_\text{theo}@f$
     */
    virtual double theoerr_FCCee240_function(const double klam, const double a, const double b, const double c) const;
    /**
     * @brief A function calculating the theoretical uncertainty in the ratio @f$\mu_{eeZH}@f$, 
     * dependent on the value of @f$\kappa_\lambda@f$, at the FCC-ee with @f$\sqrt{s}=365@f$ GeV.
     * @param[in] klam is the value of @f$\kappa_\lambda@f$, a, b and c are the coefficients of the 
     * quadratic function of klam
     * @return @f$\epsilon_\text{theo}@f$
     */
    virtual double theoerr_FCCee365_function(const double klam, const double a, const double b, const double c) const;

    /**
     * @brief A function calculating the theoretical uncertainty in the ratio @f$\mu_{eeZH}@f$,
     * dependent on the value of @f$\kappa_\lambda@f$, arising from the SMEFT/UV Model discrepancy,
     * at the FCC-ee with @f$\sqrt{s}=240@f$ GeV.
     * @param[in] klam is the value of @f$\kappa_\lambda@f$
     * @return @f$\epsilon_\text{theo}@f$
     */
    virtual double NPmismatch_FCCee240_function(const double klam) const;
    /**
     * @brief A function calculating the theoretical uncertainty in the ratio @f$\mu_{eeZH}@f$, 
     * dependent on the value of @f$\kappa_\lambda@f$, arising from the SMEFT/UV Model discrepancy,
     * at the FCC-ee with @f$\sqrt{s}=365@f$ GeV.
     * @param[in] klam is the value of @f$\kappa_\lambda@f$
     * @return @f$\epsilon_\text{theo}@f$
     */
    virtual double NPmismatch_FCCee365_function(const double klam) const;
...
// The error in the parameters multiplying the dimension-6 operator coefficients in the production cross sections.
    ...
    double theoerr_FCCee240, theoerr_FCCee365;///< Theoretical uncertainties in the FCCee Higgs cross-sections, arising from different calculation methods.
    double NPmismatch_FCCee240, NPmismatch_FCCee365;///< Mismatch between SMEFT and IDM (New Physics) predictions

    double theoerr_FCCee240_function_x0_coef;///< The coefficient of the constant term in the function describing the theoretical uncertainty in the ee->Zh FCC-ee cross-sections at sqrt{s}=240 GeV.
    double theoerr_FCCee240_function_x1_coef;///< The coefficient of the linear term in the function describing the theoretical uncertainty in the ee->Zh FCC-ee cross-sections at sqrt{s}=240 GeV.
    double theoerr_FCCee240_function_x2_coef;///< The coefficient of the quadratic term in the function describing the theoretical uncertainty in the ee->Zh FCC-ee cross-sections at sqrt{s}=240 GeV.
    double theoerr_FCCee365_function_x0_coef;///< The coefficient of the constant term in the function describing the theoretical uncertainty in the ee->Zh FCC-ee cross-sections at sqrt{s}=365 GeV.
    double theoerr_FCCee365_function_x1_coef;///< The coefficient of the linear term in the function describing the theoretical uncertainty in the ee->Zh FCC-ee cross-sections at sqrt{s}=365 GeV.
    double theoerr_FCCee365_function_x2_coef;///< The coefficient of the quadratic term in the function describing the theoretical uncertainty in the ee->Zh FCC-ee cross-sections at sqrt{s}=365 GeV.
...
//  ----------------------------------------------------------------------------
    ...
    double dZH,dZH1,dZH2,dZH3;///< Higgs self-coupling contribution to the universal resummed Higgs wave function renormalization and combinations entering in the linear, quadratic, and cubic pieces
    ...
    double cLH3d62_C1term;///< Parameter to control the inclusion of modifications of SM loops in Higgs processes due to dim 6 interactions modifying the Higgs trilinear coupling (Quadratic terms, including C1 contributions).
    
    double cLH3d63;///< Parameter to control the inclusion of modifications of SM loops in Higgs processes due to dim 6 interactions modifying the Higgs trilinear coupling (Cubic terms).
...
private:
    ...
    bool FlagLoopH3d6Quad_C1term; ///< A boolean flag that is true if including quadratic modifications (including C1 contributions) in the SM loops in Higgs observables due to the dim 6 interactions that contribute to the trilinear Higgs coupling.
    bool FlagLoopH3d6Cubi; ///< A boolean flag that is true if including cubic modifications in the SM loops in Higgs observables due to the dim 6 interactions that contribute to the trilinear Higgs coupling.
    bool FlagLoopH3d6Full; ///< A boolean flag that is true if including the full modifications in the SM loops in Higgs observables due to the dim 6 interactions that contribute to the trilinear Higgs coupling.
    ...
    bool FlagLoopHd6NoSubleading; ///< A boolean flag that is true if not including subleading corrections in self-coupling NLO effects
    bool FlagLoopHd6noWFR; ///< A boolean flag that is true if not including self-coupling NLO effects from wave function renormalization terms
    bool FlagHalfmueeZH; ///< A boolean flag which halves the predictions for the e+e- -> ZH cross-section
    bool FlagUseKlamDependentUncertainties; ///< A boolean flag that is true if using klam-dependent theoretical uncertainties in the ee->Zh cross-section predictions.
